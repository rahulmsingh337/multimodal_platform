...
> = {
  avatar_animate: {
    ...
    demo_image: "",
    freeNote:
      "Browser Web Speech API synthesizes voice. Real lip-sync needs REPLICATE_API_TOKEN.",
    icon: "💬",
  },
  text2video: {
    ...
    demo_image: "",
    freeNote:
      "Real video generation (SDXL → SVD-XT) needs REPLICATE_API_TOKEN.",
    icon: "🎬",
  },
  text2speech: {
    ...
    demo_image: "",
    freeNote:
      "Uses browser built-in Web Speech API — totally free, no API key.",
    icon: "🔊",
  },
  image_gen: {
    ...
    demo_image: "",
    freeNote:
      "Uses HF FLUX.1-schnell (free). Add HF_TOKEN to Vercel env for real images.",
    icon: "🖼️",
  },
};
...
export default function GeneratePage() {
  ...
  async function handleParse() {
    ...
    try {
      ...
      setNlp({
        ...
        negative_prompt:
          "blurry, watermark, extra limbs, deformed, low quality, jpeg artifacts",
        style_tags: ["cinematic", "photorealistic", "editorial", "studio-lit"],
        detected_intent: genType,
        tts_text:
          genType === "avatar_animate" || genType === "text2speech"
            ? prompt
            : null,
      });
    }
  ...
  async function handleRun() {
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      setTimes((p) => ({
        ...p,
        [stages[i].key]: (dur / 1000).toFixed(1) + "s",
      }));
      ...
      if (stages[i].key === "image") {
        try {
          ...
          if (r.ok) {
            ...
            setImageUrl(d.output?.image_url || null);
          }
        } catch {}
      }
    }
  ...
  async function handleDownload(url: string, ext: string, mime: string) {
    ...
    try {
      ...
      a.href = blobUrl;
      a.download = `multimodal-${genType}-${Date.now()}.${ext}`;
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-white/40 mt-1">
          NLP · Image · Speech · Animation · Video — all free
        </p>
      </div>
      ...
      <div className="mb-6 flex items-start gap-3 bg-indigo-500/5 border border-indigo-500/15 rounded-xl px-4 py-3">
        ...
        <div className="text-xs text-indigo-300/70 leading-relaxed">
          <span className="text-indigo-300 font-medium">Free tier active.</span>{" "}
          NLP is client-side. Speech uses your browser. Images use HF
          FLUX.1-schnell (add{" "}
          <code className="bg-indigo-500/10 px-1 rounded">HF_TOKEN</code> for
          real generation). Real video + avatar animation requires{" "}
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  value={genType}
                  onChange={(e) => {
                    setGenType(e.target.value as Intent);
                    reset();
                  }}
                  className="input-field"
                >
                  <option value="text2speech">
                    🔊 Text → Speech (Browser — fully free)
                  </option>
                  <option value="image_gen">
                    🖼️ Image Generation (HF FLUX — free)
                  </option>
                  <option value="avatar_animate">
                    💬 Avatar Animation (needs Replicate key)
                  </option>
                  <option value="text2video">
                    🎬 Text → Video (needs Replicate key)
                  </option>
                </select>
              </div>

              {/* Voice selector for TTS */}
              {(genType === "text2speech" || genType === "avatar_animate") &&
                voices.length > 0 && (
                  <div>
                    <label className="block text-xs text-white/50 mb-1.5">
                      Browser voice
            ...
            <div className="glass rounded-xl p-6 animate-fade-in space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {nlp.style_tags.map((t) => (
                  <span
                    key={t}
                    className="text-[11px] bg-white/5 text-white/50 rounded-md px-2 py-0.5"
                  >
                    {t}
                  </span>
                ))}
              ...
              {nlp.tts_text && (
                <div>
                  ...
                  <div className="flex items-start gap-2">
                    <p className="text-xs text-indigo-300/80 italic bg-indigo-500/5 rounded-lg px-3 py-2 flex-1">
                      "{nlp.tts_text}"
                    </p>
                    <button
                      onClick={() =>
                        speaking ? stopSpeech() : speakText(nlp.tts_text!)
                      }
                      className={clsx(
              ...
              <div className="p-4">
                {videoUrl && (
                  <video
                    ...
                    className="w-full rounded-lg bg-black max-h-56 object-contain"
                    key={videoUrl}
                  >
                  ...
                  <div className="bg-black/20 rounded-lg p-5 flex flex-col items-center gap-3">
                    ...
                    <button
                      onClick={() =>
                        speaking
                          ? stopSpeech()
                          : speakText(nlp?.tts_text || prompt)
                      }
                      className={clsx(
                        "flex items-center gap-2 text-sm font-medium rounded-xl px-5 py-2.5 transition-colors border",
                        speaking
                ...
                <div className="mt-3 flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full px-2.5 py-0.5">
                    Free tier
                  </span>
                  <span className="text-[10px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full px-2.5 py-0.5">
                    {cfg.icon} {genType.replace("_", " ")}
                  </span>
