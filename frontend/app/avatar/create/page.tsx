...
export default function AvatarPage() {
    ...
    const iv = setInterval(() => {
      ...
      if (prog >= 100) {
        ...
        setAvatars((p) =>
          p.map((a) =>
            a.id === newAv.id
              ? { ...a, status: "ready", lora: true, progress: 100 }
              : a,
          ),
        );
        setUploading(false);
        ...
        setAvatars((p) =>
          p.map((a) =>
            a.id === newAv.id ? { ...a, progress: Math.round(prog) } : a,
          ),
        );
  ...
  async function handleDownload(av: AvatarItem) {
    ...
    try {
      const res = await fetch(av.imageUrl);
      const blob = await res.blob();
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `avatar-${av.name}-${Date.now()}.jpg`;
      a.click();
      URL.revokeObjectURL(a.href);
    } catch {
      window.open(av.imageUrl, "_blank");
    }
  }
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-white/40 mt-1">
          Upload · Train LoRA · Animate with voice
        </p>
      </div>
        ...
        <div className="col-span-2">
          <div className="glass rounded-xl p-6">
            ...
            <div
              ...
              className={clsx(
                "border-2 border-dashed rounded-xl cursor-pointer transition-all mb-4",
                preview
                  ? "border-indigo-500/30 bg-indigo-500/5 p-1"
                  : "border-white/10 hover:border-white/20 bg-white/[0.02] p-8 flex flex-col items-center",
              )}
            >
              {preview ? (
                <img
                ...
                <>
                  ...
                  <div className="text-xs text-white/20 mt-1 text-center">
                    JPG/PNG · 512px min · face required
                  </div>
                </>
              )}
            </div>
              ...
              <div key={av.id} className="glass rounded-xl overflow-hidden">
                <div className="relative">
                  {av.imageUrl ? (
                    ...
                    <div className="w-full h-32 bg-gradient-to-br from-indigo-500/20 to-purple-600/20 flex items-center justify-center text-4xl">
                      {av.emoji}
                    </div>
                  )}
                  <div className="absolute top-2 right-2 flex gap-1">
                    {av.status === "ready" && (
                      <button
                        ...
                        className="w-6 h-6 bg-black/60 hover:bg-black/80 rounded-md flex items-center justify-center transition-colors"
                        title="Download"
                      >
                    ...
                    <button
                      ...
                      className="w-6 h-6 bg-black/60 hover:bg-red-500/80 rounded-md flex items-center justify-center transition-colors"
                      title="Delete"
                    >
