...
const SEED_JOBS: Job[] = [
  {
    ...
    type: "avatar_animate",
    status: "done",
    prompt: "Welcome to our platform. Today I will show you AI generation.",
  ...
  },
  {
    ...
    type: "text2speech",
    status: "done",
    prompt: "Hello world, this is an ElevenLabs synthesis test.",
    created: Date.now() - 720000,
    elapsed: 8,
  },
  {
    id: "job_j0k1l2",
    type: "lora_train",
    status: "done",
    prompt: "LoRA training: Priya avatar v2",
  ...
  },
  {
    ...
    type: "image_gen",
    status: "failed",
    prompt: "Cinematic portrait, SDXL + LoRA conditioning",
    created: Date.now() - 7200000,
    error: "Replicate cold start timeout (60s exceeded)",
  },
  {
    id: "job_p6q7r8",
    type: "avatar_animate",
    status: "queued",
    prompt: "Product demo walkthrough, professional tone",
        ...
        prev.map((j) => {
          ...
          if (prog >= 100)
            return {
              ...j,
              status: "done",
              elapsed: Math.round((Date.now() - j.created) / 1000),
              progress: 100,
            };
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={clsx(
              "flex items-center gap-2 text-xs text-white/40 hover:text-white/70 border border-white/10 rounded-lg px-3 py-2 transition-colors",
      ...
      <div className="flex gap-2 mb-5">
        {FILTER_OPTIONS.map((f) => (
          ...
          >
            {f === "all"
              ? `All (${jobs.length})`
              : `${f} (${jobs.filter((j) => j.status === f).length})`}
          </button>
        ))}
      </div>
      ...
      <div
        className={clsx(
          "grid gap-5",
          selectedJob ? "grid-cols-3" : "grid-cols-1",
        )}
      >
        {/* Job list */}
        <div
          className={clsx(
            selectedJob ? "col-span-2" : "col-span-1",
            "space-y-2",
          )}
        >
          {filtered.length === 0 && (
            <div className="glass rounded-xl p-12 text-center text-white/30 text-sm">
              No jobs found.
            </div>
          )}
          {filtered.map((j) => (
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={clsx(
                        "text-[10px] font-medium rounded-full px-2 py-0.5 border flex items-center gap-1",
                  ...
                  {j.prompt && (
                    <p className="text-xs text-white/40 truncate">{j.prompt}</p>
                  )}
        ...
        {selectedJob && (
          <div className="col-span-1 glass rounded-xl p-6 h-fit sticky top-8 animate-slide-up">
            ...
            <div className="space-y-4">
              ...
              <Row k="Created">
                {new Date(selectedJob.created).toLocaleTimeString()}
              </Row>
              {selectedJob.elapsed && (
                <Row k="Duration">{selectedJob.elapsed}s</Row>
              )}
              {selectedJob.prompt && (
                <div>
              ...
              {selectedJob.status === "done" && (
                <div className="rounded-lg bg-emerald-400/5 border border-emerald-400/20 p-3">
                  ...
                  <button className="mt-2 text-[11px] text-emerald-400 hover:text-emerald-300">
                    Get signed URL →
                  </button>
                </div>
              )}
