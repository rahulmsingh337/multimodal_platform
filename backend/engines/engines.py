...
class ImageEngine:
    async def generate_avatar(
        self, prompt: str, negative_prompt: str, lora_weights_url: str, seed: int = -1
    ...
    async def image_to_image(
        self, source_url: str, prompt: str, strength: float = 0.7
    ) -> str:
        output = await asyncio.to_thread(
            replicate.run,
...
class AnimationEngine:
    ...
    async def animate(
        self, avatar_image_url: str, speech_text: str, voice_id: str
    ) -> AnimationResult:
        ...
        return AnimationResult(
            video_url=video_url, audio_url=audio_url, duration_s=duration
        )
