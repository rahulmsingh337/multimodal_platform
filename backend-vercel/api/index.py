...
def free_nlp_enhance(prompt: str, gen_type: str) -> dict:
    ...
    style_map = {
        "avatar_animate": ["professional","natural lighting","sharp focus","studio quality"],
        "text2video":     ["cinematic","motion blur","temporal coherence","film quality"],
...
def db_get_avatars():
    conn = get_db()
    if not conn: return list(_avatars.values())
    try:
        cur = conn.cursor()
        cur.execute("SELECT id,name,status,lora_ready,voice_id,image_url,created_at FROM avatars ORDER BY created_at DESC")
        return [{"id":r[0],"name":r[1],"status":r[2],"lora_ready":r[3],"voice_id":r[4],"image_url":r[5],"created_at":str(r[6])} for r in cur.fetchall()]
    except: return list(_avatars.values())
    finally: conn.close()

def db_get_avatar(av_id):
    conn = get_db()
    if not conn: return _avatars.get(av_id)
    try:
        cur = conn.cursor()
        cur.execute("SELECT id,name,status,lora_ready,voice_id,image_url,created_at FROM avatars WHERE id=%s",(av_id,))
        r = cur.fetchone()
        return {"id":r[0],"name":r[1],"status":r[2],"lora_ready":r[3],"voice_id":r[4],"image_url":r[5],"created_at":str(r[6])} if r else None
    except: return _avatars.get(av_id)
    finally: conn.close()
...
def db_upsert_avatar(av):
    ...
    try:
        cur = conn.cursor()
        cur.execute("INSERT INTO avatars (id,name,status,lora_ready,voice_id) VALUES (%s,%s,%s,%s,%s) ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name,status=EXCLUDED.status",
            (av["id"],av["name"],av["status"],av["lora_ready"],av.get("voice_id","rachel")))
        conn.commit()
    except: pass
    finally: conn.close()
...
def db_create_job(job):
    ...
    try:
        ...
        cur.execute("INSERT INTO jobs (id,type,status,avatar_id,prompt,output) VALUES (%s,%s,%s,%s,%s,%s)",
            (job["id"],job["type"],job["status"],job.get("avatar_id"),job.get("prompt"),json.dumps(job.get("output")) if job.get("output") else None))
        conn.commit()
    except: pass
    finally: conn.close()
...
def db_get_jobs(limit=20):
    conn = get_db()
    if not conn: return list(_jobs.values())[-limit:]
    try:
        cur = conn.cursor()
        ...
        return [{"id":r[0],"type":r[1],"status":r[2],"avatar_id":r[3],"prompt":r[4],"output":r[5],"error":r[6],"created_at":str(r[7])} for r in cur.fetchall()]
    except: return list(_jobs.values())
    finally: conn.close()

def db_get_job(job_id):
    conn = get_db()
    if not conn: return _jobs.get(job_id)
    try:
        cur = conn.cursor()
    ...
    except: return _jobs.get(job_id)
    finally: conn.close()
...
async def list_avatars():
    a = db_get_avatars(); return {"avatars":a,"total":len(a)}

@app.get("/api/v1/avatars/{avatar_id}")
async def get_avatar(avatar_id: str):
    av = db_get_avatar(avatar_id)
    if not av: raise HTTPException(404,"Not found")
    return av
...
async def create_avatar(name: str = "New Avatar", voice_id: str = "rachel"):
    ...
    av = {"id":av_id,"name":name,"status":"training","lora_ready":False,"voice_id":voice_id,"image_url":None,"created_at":datetime.utcnow().isoformat()}
    job = {"id":job_id,"type":"lora_train","status":"queued","avatar_id":av_id,"prompt":f"LoRA: {name}","output":None,"error":None,"created_at":datetime.utcnow().isoformat()}
    db_upsert_avatar(av); db_create_job(job)
...
async def delete_avatar(avatar_id: str):
    ...
    conn = get_db()
    if conn:
        try: cur = conn.cursor(); cur.execute("DELETE FROM avatars WHERE id=%s",(avatar_id,)); conn.commit()
        finally: conn.close()
    return {"deleted":True}
...
async def animate_avatar(avatar_id: str, text: str = "Hello", voice_id: str = "rachel"):
    job_id = "job_" + str(uuid.uuid4())[:8]
    output = {"video_url":"https://www.w3schools.com/html/mov_bbb.mp4","audio_url":"https://www.w3schools.com/html/horse.mp3","duration_s":10.0}
    job = {"id":job_id,"type":"avatar_animate","status":"done","avatar_id":avatar_id,"prompt":text,"output":output,"error":None,"created_at":datetime.utcnow().isoformat()}
    db_create_job(job); return {"job_id":job_id,"status":"done","output":output}

@app.post("/api/v1/generate/prompt-refine")
async def refine_prompt(body: dict):
    raw = body.get("raw_prompt",""); gt = body.get("context",{}).get("type","avatar_animate")
...
async def text2video(body: dict):
    ...
    job = {"id":job_id,"type":"text2video","status":"done","prompt":body.get("raw_prompt",""),"output":output,"error":None,"created_at":datetime.utcnow().isoformat()}
    db_create_job(job); return {"job_id":job_id,"status":"done","output":output}
...
async def text2speech(body: dict):
    ...
    job = {"id":job_id,"type":"text2speech","status":"done","prompt":body.get("text",""),"output":output,"error":None,"created_at":datetime.utcnow().isoformat()}
    db_create_job(job); return {"job_id":job_id,"status":"done","output":output}
...
async def gen_image(body: dict):
    ...
    output = {"image_url":image_url,"model":"FLUX.1-schnell (free)" if HF_TOKEN else "placeholder"}
    job = {"id":job_id,"type":"image_gen","status":"done","prompt":prompt,"output":output,"error":None,"created_at":datetime.utcnow().isoformat()}
    db_create_job(job); return {"job_id":job_id,"status":"done","output":output}

@app.get("/api/v1/jobs/")
async def list_jobs(page: int = 1, limit: int = 20):
    jobs = db_get_jobs(limit); return {"page":page,"limit":limit,"total":len(jobs),"jobs":jobs}
...
async def cancel_job(job_id: str):
    if job_id in _jobs: _jobs[job_id]["status"] = "cancelled"
    conn = get_db()
    if conn:
        try: cur = conn.cursor(); cur.execute("UPDATE jobs SET status='cancelled' WHERE id=%s",(job_id,)); conn.commit()
        finally: conn.close()
...
async def storage_status():
    return {
        "postgres":{"connected":bool(get_db()),"provider":"Vercel Postgres (free)","env_var":"POSTGRES_URL"},
        "ai_nlp":{"connected":True,"provider":"client-side (zero cost)"},
        "ai_image":{"connected":bool(HF_TOKEN),"provider":"Hugging Face free tier","env_var":"HF_TOKEN"},
